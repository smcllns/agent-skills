...
//   - Fixtures live in `markdown-agent-directives.spec.md`. Each section's fenced
...
beforeAll(async () => {
  const grepScan = await writeFixtures(ALL_FIXTURES, "markdown-agent-directives-grep-");
  ...
  const doneScan = await writeFixtures(DONE_FIXTURES, "markdown-agent-directives-done-");
