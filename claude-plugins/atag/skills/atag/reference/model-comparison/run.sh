...
# then invokes `claude -p --model <model>` asking it to resolve all `atag`
# tags in that file. Inspect the result files by eye.
...
for model in "$@"; do
  ...
  claude -p --model "$model" "Run the atag skill on the file at ${abs_out} — resolve every unresolved tag in that single file. Do not touch any other files."
